...
final class SearchSamplingStratigraphicUnitFilter extends AbstractFilter
{
    ...
    protected function filterProperty(string $property, $value, QueryBuilder $queryBuilder, QueryNameGeneratorInterface $queryNameGenerator, string $resourceClass, ?Operation $operation = null, array $context = []): void
    {
        ...
        if (1 === $chunkCount) {
            $chunk = $chunks[0];
            if (is_numeric($chunk)) {
                // a.2: is numeric -> filter by number
            ...
            // 3+ chunks: not supported for sampling SU (no year field) -> return empty set
            $whereConditions[] = $this->createEmptySetCondition($queryBuilder, $siteAlias);
        }

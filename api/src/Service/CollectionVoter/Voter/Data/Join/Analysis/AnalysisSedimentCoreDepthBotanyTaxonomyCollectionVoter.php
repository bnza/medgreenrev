...
readonly class AnalysisSedimentCoreDepthBotanyTaxonomyCollectionVoter extends AbstractCollectionVoter
{
    protected function voteOnSubCollection(object $parent, TokenInterface $token): bool
    {
        $user = $token->getUser();
        if ($parent instanceof AnalysisSedimentCoreDepthBotany && $user instanceof User) {
            return $this->accessDecisionManager->decide($token, ['ROLE_ARCHAEOBOTANIST'])
                && $this->sitePrivilegeManager->hasSitePrivileges($user, $parent->getSubject()->getSedimentCore()?->getSite());
        }

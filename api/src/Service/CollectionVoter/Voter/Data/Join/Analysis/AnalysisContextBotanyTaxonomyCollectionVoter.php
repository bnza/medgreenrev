...
readonly class AnalysisContextBotanyTaxonomyCollectionVoter extends AbstractCollectionVoter
{
    protected function voteOnSubCollection(object $parent, TokenInterface $token): bool
    {
        $user = $token->getUser();
        if ($parent instanceof AnalysisContextBotany && $user instanceof User) {
            return $this->accessDecisionManager->decide($token, ['ROLE_ARCHAEOBOTANIST'])
                && $this->sitePrivilegeManager->hasSitePrivileges($user, $parent->getSubject()->getSite());
        }

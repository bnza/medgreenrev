...
readonly class AnalysisSedimentCoreDepthBotanyCollectionVoter extends AbstractCollectionVoter
{
    protected function voteOnSubCollection(object $parent, TokenInterface $token): bool
    {
        ...
        if ($parent instanceof SedimentCoreDepth && $user instanceof User) {
            return $this->accessDecisionManager->decide($token, ['ROLE_ARCHAEOBOTANIST'])
                && $this->sitePrivilegeManager->hasSitePrivileges($user, $parent->getSedimentCore()?->getSite());
        }

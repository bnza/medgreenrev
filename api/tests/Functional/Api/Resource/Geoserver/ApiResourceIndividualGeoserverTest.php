...
class ApiResourceIndividualGeoserverTest extends ApiTestCase
{
    ...
    public function testGetCollectionJsonFiltered(): void
    {
        ...
        $unfilteredResponse = $this->apiRequest($client, 'GET', '/api/features/individuals', [
            'token' => $token,
            'headers' => [
                'Accept' => 'application/json',
            ],
    ...
    public function testAggregatedFeatureCollectionFilterConsistency(): void
    {
        ...
        // Step 1a: Discover a valid sex filter value from existing data
        $response = $this->apiRequest($client, 'GET', '/api/data/individuals?page=1&itemsPerPage=100');
        $this->assertResponseStatusCodeSame(200);
        $members = json_decode($response->getContent(), true)['member'];
        $this->assertNotEmpty($members, 'Need at least one individual in fixtures');
        ...
        $this->assertNotNull($sexIri, 'Need at least one individual with sex in fixtures');

        // Step 1b: Fetch all individuals filtered by that sex
        $filteredResponse = $this->apiRequest($client, 'GET', '/api/data/individuals?page=1&itemsPerPage=500&sex[0]='.$sexIri);
        $this->assertResponseStatusCodeSame(200);
        $filteredData = json_decode($filteredResponse->getContent(), true);
        $filteredMembers = $filteredData['member'];
        $this->assertNotEmpty($filteredMembers);
        ...
        $aggResponse = $this->apiRequest($client, 'GET', '/api/features/individuals?sex[0]='.$sexIri, [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ]);

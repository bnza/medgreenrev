...
class ApiResourceBotanyCharcoalGeoserverTest extends ApiTestCase
{
    ...
    public function testGetCollectionJsonFiltered(): void
    {
        ...
        $unfilteredResponse = $this->apiRequest($client, 'GET', '/api/features/botany/charcoals', [
            'token' => $token,
            'headers' => [
                'Accept' => 'application/json',
            ],
    ...
    public function testAggregatedFeatureCollectionFilterConsistency(): void
    {
        ...
        $filterParam = 'exists[analyses]=true';

        // Step 1: Fetch all charcoals filtered by exists[analyses]=true
        $filteredResponse = $this->apiRequest($client, 'GET', '/api/data/botany/charcoals?page=1&itemsPerPage=500&'.$filterParam);
        $this->assertResponseStatusCodeSame(200);
        $filteredData = json_decode($filteredResponse->getContent(), true);
        $filteredMembers = $filteredData['member'];
        $this->assertNotEmpty($filteredMembers, 'Need at least one charcoal with analyses in fixtures');
        ...
        $aggResponse = $this->apiRequest($client, 'GET', '/api/features/botany/charcoals?'.$filterParam, [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ]);

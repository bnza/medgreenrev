...
class ApiResourceSamplingStratigraphicUnitGeoserverTest extends ApiTestCase
{
    ...
    public function testGetCollectionJsonUnfiltered(): void
    {
        ...
        $collectionResponse = $this->apiRequest($client, 'GET', '/api/features/sampling_stratigraphic_units', [
            'token' => $token,
            'headers' => [
    ...
    public function testGetCollectionJsonFiltered(): void
    {
        ...
        $unfilteredResponse = $this->apiRequest($client, 'GET', '/api/features/sampling_stratigraphic_units', [
            'token' => $token,
            'headers' => [
                'Accept' => 'application/json',
            ],
        ...
        // Get filtered counts
        $collectionResponse = $this->apiRequest($client, 'GET', '/api/features/sampling_stratigraphic_units?number=1', [
            'token' => $token,
            'headers' => [
                'Accept' => 'application/json',
            ],
    ...
    public function testGetCollectionGeoJson(): void
    {
        ...
        $this->assertStringStartsWith('sampling_stratigraphic_units:', $firstFeature['id']);
    }
    ...
    public function testGetNumberMatched(): void
    {
        ...
        $collectionResponse = $this->apiRequest($client, 'GET', '/api/features/number_matched/sampling_stratigraphic_units', [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
    ...
    public function testGetExtentMatched(): void
    {
        ...
        $collectionResponse = $this->apiRequest($client, 'GET', '/api/features/extent_matched/sampling_stratigraphic_units', [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
        $responseArray = $collectionResponse->toArray();
    ...
    public function testAggregatedFeatureCollectionFilterConsistency(): void
    {
        ...
        $filterParam = 'number=601';

        // Step 1: Fetch all sampling stratigraphic units filtered by number
        $filteredResponse = $this->apiRequest($client, 'GET', '/api/data/sampling_stratigraphic_units?page=1&itemsPerPage=500&'.$filterParam);
        $this->assertResponseStatusCodeSame(200);
        $filteredData = json_decode($filteredResponse->getContent(), true);
        $filteredMembers = $filteredData['member'];
        $this->assertNotEmpty($filteredMembers, 'Need at least one sampling stratigraphic unit with number 601 in fixtures');

        // Step 2: Build expected {siteId => count} from data endpoint
        $expectedCounts = [];
        $expectedSiteCodes = [];
        foreach ($filteredMembers as $member) {
            $siteIri = $member['site']['@id'];
            $siteId = (int) basename($siteIri);
            $expectedCounts[$siteId] = ($expectedCounts[$siteId] ?? 0) + 1;
            $expectedSiteCodes[$siteId] = $member['site']['code'];
        }

        // Step 3: Verify sites exist via the sites endpoint
        $codeParams = array_map(fn ($c) => 'code[]='.$c, array_unique($expectedSiteCodes));
        $sitesResponse = $this->apiRequest($client, 'GET', '/api/data/sampling_sites?page=1&itemsPerPage=100&'.implode('&', $codeParams));
        $this->assertResponseStatusCodeSame(200);
        ...
        foreach (array_unique($expectedSiteCodes) as $code) {
            $this->assertContains($code, $returnedSiteCodes, "Site code '$code' not found in sampling_sites endpoint");
        }

        // Step 4: Call aggregated endpoint with same filter
        $aggResponse = $this->apiRequest($client, 'GET', '/api/features/sampling_stratigraphic_units?'.$filterParam, [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ]);
    ...
    public function testGetExport(): void
    {
        ...
        $collectionResponse = $this->apiRequest($client, 'GET', '/api/features/export/sampling_stratigraphic_units', [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
    }

...
class ApiResourceBotanySeedGeoserverTest extends ApiTestCase
{
    ...
    public function testGetCollectionJsonFiltered(): void
    {
        ...
        $unfilteredResponse = $this->apiRequest($client, 'GET', '/api/features/botany/seeds', [
            'token' => $token,
            'headers' => [
                'Accept' => 'application/json',
            ],
    ...
    public function testAggregatedFeatureCollectionFilterConsistency(): void
    {
        ...
        // Step 1: Fetch all seeds filtered by exists[analyses]=true
        $filteredResponse = $this->apiRequest($client, 'GET', '/api/data/botany/seeds?page=1&itemsPerPage=500&'.$filterParam);
        $this->assertResponseStatusCodeSame(200);
        $filteredData = json_decode($filteredResponse->getContent(), true);
        $filteredMembers = $filteredData['member'];
        $this->assertNotEmpty($filteredMembers, 'Need at least one seed with analyses in fixtures');
        ...
        $aggResponse = $this->apiRequest($client, 'GET', '/api/features/botany/seeds?'.$filterParam, [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ]);

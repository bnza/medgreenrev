...
class ApiResourceZooToothGeoserverTest extends ApiTestCase
{
    ...
    public function testGetCollectionJsonFiltered(): void
    {
        ...
        $unfilteredResponse = $this->apiRequest($client, 'GET', '/api/features/zoo/teeth', [
            'token' => $token,
            'headers' => [
                'Accept' => 'application/json',
            ],
    ...
    public function testAggregatedFeatureCollectionFilterConsistency(): void
    {
        ...
        // Step 1: Fetch all teeth filtered by exists[analyses]=true
        $filteredResponse = $this->apiRequest($client, 'GET', '/api/data/zoo/teeth?page=1&itemsPerPage=500&'.$filterParam);
        $this->assertResponseStatusCodeSame(200);
        $filteredData = json_decode($filteredResponse->getContent(), true);
        $filteredMembers = $filteredData['member'];
        $this->assertNotEmpty($filteredMembers, 'Need at least one tooth with analyses in fixtures');
        ...
        $aggResponse = $this->apiRequest($client, 'GET', '/api/features/zoo/teeth?'.$filterParam, [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ]);

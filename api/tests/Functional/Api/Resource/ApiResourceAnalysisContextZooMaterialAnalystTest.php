...
class ApiResourceAnalysisContextZooMaterialAnalystTest extends ApiTestCase
{
    ...
    public function testPostGetCollectionParentSubjectAclReturnsTrueForMaterialAnalystUser(): void
    {
        ...
        $subjectResponse = $this->apiRequest($client, 'GET', '/api/data/contexts?itemsPerPage=1', ['token' => $adminToken]);
        ...
        $siteIri = $subject['site']['@id'];

        // 3. Create a material analyst user and grant site privilege
        $email = 'mat_analyst_context_zoo_test@example.com';
        ...
        $collectionResponse = $this->apiRequest($client, 'GET', "/api/data/contexts/$subjectId/analyses/zoo", ['token' => $matToken]);
